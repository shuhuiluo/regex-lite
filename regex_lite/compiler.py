...
def _new_state(states: List[State], accept=False) -> int:
    states.append(State(accept=accept)); return len(states)-1
...
def _build(node: ast.Expr, states: List[State]) -> Tuple[int,int]:
    if isinstance(node, ast.Empty):
        s = _new_state(states); a = _new_state(states, True); _link_eps(states, s, a); return s,a
    if isinstance(node, ast.Literal):
        s = _new_state(states); a = _new_state(states, True); _link_char(states, s, node.char, a); return s,a
